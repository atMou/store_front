...
export function buildProductFormData(form: ProductFormSchema) {
  ...
  // Top-level Images (append each file with same key)
  for (const file of form.images || []) {
    fd.append("Images", file);
  }

  // Top-level IsMain booleans with indexed notation [0], [1], etc.
  (form.isMain || []).forEach((b, index) => {
    fd.append(`IsMain[${index}]`, String(b));
  });

  // Variants: use PascalCase and indexed notation
  (form.variants || []).forEach((v, i) => {
    fd.append(`Variants[${i}].Color`, v.color);

    // Variant Images: append each file with same nested key
    (v.images || []).forEach((file) => {
      fd.append(`Variants[${i}].Images`, file);
    });

    // Variant IsMain with indexed notation [0], [1], etc.
...
export const buildUpdateProductFormData = (form: UpdateProductFormSchema) => {
  ...
  // Existing images to keep (required IEnumerable on backend)
  ...
  // New Images (nullable on backend)
  ...
  // Don't append anything for nullable empty arrays

  // New Images IsMain flags (nullable on backend)
  ...
  (form.variants || []).forEach((v, i) => {
    ...
    // Size Variants (only for new variants)
    ...
    // Existing variant images to keep (required IEnumerable on backend)
    const variantImageDtos = v.imageDtos || [];
    if (variantImageDtos.length === 0) {
      // For empty required IEnumerable in variants, append indexed marker
    ...
    // New variant images (nullable on backend)
    ...
    // New variant images IsMain flags (nullable on backend)
  ...
  // Alternative product IDs (required IEnumerable on backend)
  const alternatives = form.alternativesIds || [];
  alternatives.forEach((id, i) => {
    fd.append(`AlternativesIds[${i}]`, id);
  });
  // Note: Empty IEnumerable will be bound as empty collection by .NET
